  ...
  <div>
      ...
      <div class="relative z-10 w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid md:grid-cols-2 gap-10 items-center">
          <div class="space-y-6 text-white">
            <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 border border-white/15 text-xs uppercase tracking-[0.25em]">
              {{ hero.eyebrow }}
            </div>
            <div class="space-y-3">
            ...
            <ul class="space-y-3 text-white/90">
              ...
              >
                <span class="flex h-9 w-9 items-center justify-center rounded-full bg-white/10 border border-white/10">
          ...
          <div class="bg-white/10 rounded-2xl p-6 md:p-7 border border-white/15 shadow-2xl shadow-black/20 backdrop-blur-md">
            <p class="text-sm uppercase tracking-[0.25em] text-white/70 mb-4">{{ highlights.title }}</p>
            <div class="space-y-4">
              <div class="flex items-start gap-4" v-for="item in highlights.items" :key="item.title">
                <span class="flex h-10 w-10 items-center justify-center rounded-xl bg-white/10 border border-white/15">
            ...
            <div class="mt-6 flex items-center justify-between text-white/80 text-sm border-t border-white/10 pt-4">
              <span>{{ highlights.footer }}</span>
              <span class="inline-flex items-center gap-2 text-white">
                <span class="h-2 w-2 rounded-full bg-brand-orange animate-pulse"></span> {{ highlights.status }}
              </span>
            </div>
          </div>
        </div>
      ...
      <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        <section class="space-y-6">
          <div class="text-center space-y-3">
            <p class="text-sm uppercase tracking-[0.35em] text-gray-400">{{ how.eyebrow }}</p>
            <h2 class="text-3xl md:text-4xl font-bold text-gray-900">{{ how.title }}</h2>
          ...
          <div class="grid md:grid-cols-3 gap-4">
            ...
            >
              <div class="inline-flex items-center justify-center h-9 w-9 rounded-full bg-main/10 text-main font-semibold">{{ idx + 1 }}</div>
        ...
        <div v-else>
          <div class="bg-white/95 backdrop-blur-sm rounded-[32px] shadow-2xl p-6 md:p-8 border border-gray-100">
            ...
            <div class="text-center mb-8 space-y-2">
              <h2 class="text-3xl md:text-4xl font-bold text-gray-900">{{ currentMonthName }} {{ currentYear }}</h2>
              <p class="text-gray-600 text-sm md:text-base">{{ calendarCopy.helper }}</p>
              <div class="flex justify-between items-center max-w-md mx-auto mt-4">
                <button 
                  @click="previousMonth"
            ...
            <div class="grid grid-cols-7 gap-2">
              <div 
                ...
                :key="day.date"
                :class="[
                  'min-h-24 md:min-h-32 p-3 border rounded-2xl cursor-pointer transition-all hover:-translate-y-[1px]',
                  day.isToday
                  ...
                  day.hasEvents && !day.isToday ? 'border-brand-orange/50 shadow-sm' : ''
                ]"
              ...
              >
                <div class="flex items-start justify-between mb-2">
                  ...
                  <div v-if="day.events && day.events.length > 0" class="text-[11px] font-semibold rounded-full px-2 py-0.5"
                    :class="day.isToday ? 'bg-white/20 text-white' : 'bg-brand-orange/10 text-brand-orange'">
          ...
          <div v-if="selectedDateEvents.length > 0" class="bg-white/95 rounded-[32px] shadow-2xl p-6 md:p-8 mt-8 border border-gray-100">
            <h3 class="text-2xl md:text-3xl font-bold text-gray-900 mb-6">
              {{ calendarCopy.selectedPrefix }} {{ formatFullDate(selectedDay) }}
            </h3>
            <div class="space-y-4">
              <div 
    ...
    >
      ...
      >
        ...
        <div class="sticky top-0 bg-gradient-to-r from-main to-secondary px-8 py-6 flex justify-between items-start z-10">
          <div class="flex-1">
            ...
            <div class="flex flex-wrap items-center gap-3 text-sm">
              <div class="flex items-center gap-2 text-white/90 px-3 py-1 rounded-full bg-white/10 border border-white/10">
        ...
        <!-- Modal Content -->
        <div class="px-8 py-8 overflow-y-auto max-h-[calc(90vh-180px)] bg-gray-50">
    ...
    const translations = {
      en: {
        hero: {
          ...
          subtitle: 'One place to see every worship gathering, prayer night, outreach, and family moment we have planned.',
      ...
      kh: {
        hero: {
          ...
          subtitle: 'មើលសកម្មភាពសប្ដាហ៍ ព្រឹត្តិការណ៍ជាក់លាក់ និងការជួបជុំគ្រួសារទាំងអស់នៅកន្លែងតែមួយ។',
          chips: ['កាលវិភាគប្រចាំខែ', 'ថ្វាយបង្គំ និងអធិស្ឋាន', 'យុវជន និងគ្រួសារ'],
          checklist: [
        ...
        highlights: {
          ...
          status: 'សកម្ម',
          items: [
            { title: 'សេចក្ដីថ្វាយបង្គំព្រះអាទិត្យ', detail: 'រៀងរាល់អាទិត្យ · ម៉ោង ៩:០០ ព្រឹក' },
            { title: 'ក្រុមយុវជន', detail: 'រៀងរាល់សៅរ៍ · ម៉ោង ៦:០០ ល្ងាច' },
            { title: 'យប់អធិស្ឋាន', detail: 'សៅរ៍ដំបូងប្រចាំខែ · ម៉ោង ៧:០០ ល្ងាច' }
          ]
        },
        how: {
          ...
          steps: [
            { title: 'រុករកតាមខែ', body: 'ប្រើប៊ូតុងខែមុន/ខែក្រោយ ឬ "ថ្ងៃនេះ" ដើម្បីត្រឡប់ទៅខែបច្ចុប្បន្ន។' },
            { title: 'ជ្រើសថ្ងៃដែលបន្លិច', body: 'ថ្ងៃដែលបន្លិចបង្ហាញចំនួនព្រឹត្តិការណ៍ក្នុងថ្ងៃនោះ។' },
            { title: 'បើកព្រឹត្តិការណ៍', body: 'មើលម៉ោង ទីតាំង និងការពិពណ៌នា ហើយចែករំលែកព័ត៌មានជាមួយមិត្តភក្តិ។' }
          ]
        },
        calendar: {
          helper: 'ជ្រើសថ្ងៃដែលបន្លិចដើម្បីមើលព្រឹត្តិការណ៍ និងព័ត៌មានលម្អិត។',
        ...
        events: {
          sunday: {
            ...
            summary: 'ចូលរួមរៀងរាល់អាទិត្យសម្រាប់ការថ្វាយបង្គំ អធិស្ឋាន និងសមាគម។',
          ...
          baptism: {
            ...
            summary: 'ពិធីជ្រមុជទឹកជាសាធារណៈ និងអបអរសាទរជីវិតថ្មីក្នុងព្រះគ្រីស្ទ។',
